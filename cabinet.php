	...
	<div id="block_one_cabinet">
		<form action="#" method="post" id="form">
			<div id="panel">
				...
				<input type="button" name="history" id="but_admin"  value="Админ" class="button <?php echo ($_SESSION[login] == 'VIEWER')? '':'display_none' ?>">
				<input type="button" name="key" id="but_key"  value="Ключи" class="button <?php echo ($_SESSION[login] == 'VIEWER')? '':'display_none' ?>">
				<input type="button" name="cupon" id="but_cupon"  value="Купон" class="button <?php echo ($_SESSION[login] == 'VIEWER')? '':'display_none' ?>">
			</div>
	...
	<div id="block_two_cabinet">
		...
		<div id="key" class="dis_none">
			...
			<form action="#" method="post">
				<div class="editing_edit">
					<label>
						Связь(id товара и ключей)
						<input type="number" name="tovar_num_svaz">
					</label>
			...
			<form action="#" method="post">
				<div class="editing_edit">
					<label>
						Ключ к товару
						<input type="number" name="tovar_num_key">
					</label>
		...
		<div id="cupon" class="dis_none">
			...
			<form action="#" method="post">
				...
				<input type="text" name="cupon_name">
				<input type="number" name="cupon_num">
				<input type="submit" name="cupon_create">
			</form>
			<hr>
			<?php
			$cuponread = $tovar->cuponread();
			while ($cuponreads = $cuponread->fetch(PDO::FETCH_ASSOC)) {
			?>
			<div>
				<form action="#" method="post">
					<label>
						<?php echo $cuponreads[cupon] ?>
					</label>
					<label>
						<?php echo "-".$cuponreads[discount]."%" ?>
					</label>
					<img src="content/icon/cross.png" class="crosss">
					<input class="crosss"  type="submit" name="delet" value="<?php echo $cuponreads[id] ?>">
				</form>
...
if (isset($_POST['delet'])) {
	$DeletCupon = $tovar->DeletCupon($_POST['delet']);
}
if (isset($_POST['cupon_create'])) {
	$cuponread = $tovar->cuponread();
	$chet = null;
	while ($cuponreads = $cuponread->fetch(PDO::FETCH_ASSOC)) {
		if($cuponreads[cupon] == mb_strtoupper($_POST[cupon_name])){
			$chet = 1;
		}
	}
	if($chet != 1){
		$cuponcreate = $tovar->cuponcreate(mb_strtoupper($_POST[cupon_name]),$_POST[cupon_num]);
	}
}

if (isset($_POST['key_but_delet'])) {
	$DeletKey = $tovar->DeletKey($_POST[tovar_num_svaz]);
}
if (isset($_POST['key_but_key'])) {
	$chet = null;
	$DeletKey = $tovar->DeletKey($_POST[tovar_num_svaz]);
	foreach ($_POST['tovar'] as $key) {
		$readTovarKey = $tovar->readTovarKey($_POST[tovar_num_svaz]);
		...
		if ($chet == $key) {
			...
			$UpdateTovarKey = $tovar->UpdateTovarKey($_POST[tovar_num_svaz],$key);
		}
	}
}
if (isset($_POST['key_but'])) {
	...
	$CreateKeyTwos = $tovar->CreateKeyTwo($_POST[tovar_num_key],$SELECTKeys[id_key]);
}

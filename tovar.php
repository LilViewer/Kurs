...
<section class="vertical-center slider">
	...
	<div class=" <?php echo ($_SESSION[login] == 'VIEWER')? 'admin_blcok':'display_none' ?> ">
		<a class='button' href="editing.php?varname=<?php echo $name[id_tovar]?> ">
			Отредоктировать
		</a>
	</div>
<div id="black_slider">
			...
			<div class="janr">
				...
				<div class="janr_block_two">
					<label class="block_two">
						...
						$genress = $genre->fetchAll(PDO::FETCH_ASSOC);
						$chet = 0 ;
						$genre = $tovar->readGenre($var_value);
						while ($genres = $genre->fetch(PDO::FETCH_ASSOC)) {
							echo "<a href='genre.php?varname=$genres[id_genre]'>".$genres[name_genre]."</a>".($chet < COUNT($genress)-1? ", " : ".");
							$chet++;
						}
						?>
					</label>
					...
					<label class="block_two">
						...
						$publisherss = $publisher->fetchAll(PDO::FETCH_ASSOC);
						$chet = 0 ;
						$publisher = $tovar->readPublisher($var_value);
						while ($publishers = $publisher->fetch(PDO::FETCH_ASSOC)) {
							echo "<a href='publisher.php?varname=$publishers[id_publisher]'>".$publishers[name_publisher]."</a>".($chet < COUNT($publisherss)-1? ", " : ".");
							$chet++;
						}
						?>
					</label>
					<label class="block_two">
						...
						$developerss = $developer->fetchAll(PDO::FETCH_ASSOC);
						$chet = 0 ;
						$developer = $tovar->readDeveloper($var_value);
						while ($developers = $developer->fetch(PDO::FETCH_ASSOC)) {
							echo "<a href='developer.php?varname=$developers[id_developer]'>".$developers[name_developer]."</a>".($chet < COUNT($developerss)-1? ", " : ".");
							$chet++;
						}
						?>
					</label>
